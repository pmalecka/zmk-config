...
yaml2json "${PWD}/build.yaml" | jq -c '.include[]' | while read -r item; do
  ...
  local_cmake_args=$(echo "$item" | jq -r '.["local-cmake-args"] // empty')
  ...
  expanded_local_cmake_args=""
  if [ -n "${local_cmake_args}" ]; then
    eval "expanded_local_cmake_args=($local_cmake_args)"
  fi
  ...
  if [[ -n "$tags" ]]; then
    ...
    for tag in "${tags_array[@]}"; do
      ...
      if [ "${!env_var_name:-}" = true ] || [ "${BUILD_ALL:-}" = true ] ; then
        ...
        echo west build -p=always -s zmk/app -d "build/${artifact_name}" -b "${board}" "${extra_west_args}" -- -DZMK_CONFIG="${PWD}/config" "${extra_cmake_args}" "${expanded_cmake_args[@]}" "${expanded_local_cmake_args[@]}"
        west build -p=always -s zmk/app -d "build/${artifact_name}" -b "${board}" "${extra_west_args}" -- -DZMK_CONFIG="${PWD}/config" "${extra_cmake_args}" "${expanded_cmake_args[@]}" "${expanded_local_cmake_args[@]}"
        # Left Kconfig file
